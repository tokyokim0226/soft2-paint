...
int *index_reader_for_layer(int *len);
...
CHPENSIZE,ERASER,MARKER,COLORCHANGED,PENSIZENOTINT,PENSIZETOOBIG,COLORNOTINT,COLORNOTINRANGE ,NOCOMMAND} Result;
...
void push_back_history(History *his, char *str); //new version of push_command from kadai 3
...
void draw_dot(Canvas *c, const int x0, const int y0){
    if (x0 < 0 || x0 >= c->width || y0<0 || y0 >= c->height){
        ...
        if (c -> pen == ' ') {//when it is a marker {
            layer -> board[x0][y0] = ' ';
            layer -> bgcolor[x0][y0] = c -> color;
        }
        else if (c -> pen == 0){ //Eraser mode
            layer -> board[x0][y0] = 0;
            layer -> color[x0][y0] = 0;
            layer -> bgcolor[x0][y0] = 0;
        }
...
Result interpret_command(const char *command, History *his, Canvas *c){
    ...
    if (strcmp(s, "circle") == 0) {
        int p[4] = {0}; // p[0]: x0, p[1]: y0, p[2]: r (radius)
        char *b[4];
        for (int i = 0 ; i < 4; i++){
            b[i] = strtok(NULL, " ");
            if (b[i] == NULL){
...
int read_layer_index(int default_value) {
    int len;
    int *indices = index_reader_for_layer(&len);
...
int remove_layer(Canvas *c, int index, int freeing) {
  ...
  if (c->layer_index >= c->layer_list->size) {
